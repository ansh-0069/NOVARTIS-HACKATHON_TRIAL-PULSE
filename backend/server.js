...
async function calculateMassBalance(data, hybrid_results) {
    ...
    // Estimate volatile loss — pass gc_ms_detected so the recommendation is
    // context-aware (no point recommending a test that's already been run).
    const volatile_analysis = estimateVolatileLoss({
        ...
        gc_ms_detected: !!(data.gc_ms_detected),
        amb: amb
    });
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
// Predictive Degradation API  (pure Node.js — no Python/rdkit dependency)
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

// --- helpers -----------------------------------------------------------
function parseSmilesFeatures(smiles) {
    const s = smiles || '';
    return {
        hasEster: /C\(=O\)O[^H]/.test(s) || /OC\(=O\)/.test(s),
        hasAmide: /C\(=O\)N/.test(s) || /NC\(=O\)/.test(s),
        hasAlcohol: /[^O]O[^C=]/.test(s) || /CO[^C=]/.test(s) || /\[OH\]/.test(s),
        hasSecAlcohol: /[^O]C\(O\)[^H]/.test(s),
        hasSulfide: /CSC|cSc/.test(s),
        hasAmine: /[CN]N|N[^=]/.test(s),
        hasAromaticRing: /c1/.test(s),
        hasCarboxylicAcid: /C\(=O\)O(?!C)|C\(=O\)\[OH\]/.test(s),
        hasLactone: /OC\(=O\)|C\(=O\)O(?=C.*O)/.test(s),
        numAtoms: (s.match(/[A-Z]/g) || []).length || 5,
        molecularWeight: estimateMW(s),
    };
}

function estimateMW(smiles) {
    const atomWeights = { C: 12, N: 14, O: 16, S: 32, F: 19, Cl: 35.5, Br: 80, I: 127, P: 31, H: 1 };
    let mw = 0;
    const matches = smiles.match(/[A-Z][a-z]?/g) || [];
    for (const atom of matches) {
        mw += atomWeights[atom] || 12;
    }
    // Add implied hydrogens (rough estimate)
    mw += (smiles.replace(/[^A-Z]/g, '').length * 1.5);
    return Math.round(mw);
}

// SMILES-based rule-based degradation rules
const DEGRADATION_RULES = [
    {
        id: 'ester_hydrolysis',
        label: 'Ester hydrolysis to carboxylic acid + alcohol',
        category: 'acid_hydrolysis',
        conditions: ['acid', 'base'],
        test: f => f.hasEster,
        omegaFn: mw => parseFloat((mw / (mw - 18)).toFixed(3)),
        confidenceFn: (mw, s) => s === 'acid' ? 88 : 83,
        productSuffix: '(carboxylic acid fragment)'
    },
    {
        id: 'amide_hydrolysis',
        label: 'Amide hydrolysis to carboxylic acid + amine',
        category: 'acid_hydrolysis',
        conditions: ['acid', 'base'],
        test: f => f.hasAmide,
        omegaFn: mw => parseFloat((mw / (mw - 17)).toFixed(3)),
        confidenceFn: (mw, s) => s === 'base' ? 91 : 78,
        productSuffix: '(amine fragment)'
    },
    {
        id: 'primary_alcohol_oxidation',
        label: 'Primary alcohol oxidation to aldehyde',
        category: 'oxidation',
        conditions: ['oxidative'],
        test: f => f.hasAlcohol && !f.hasSecAlcohol,
        omegaFn: mw => parseFloat((mw / (mw - 2)).toFixed(3)),
        confidenceFn: () => 74,
        productSuffix: '(aldehyde)'
    },
    {
        id: 'secondary_alcohol_oxidation',
        label: 'Secondary alcohol oxidation to ketone',
        category: 'oxidation',
        conditions: ['oxidative'],
        test: f => f.hasSecAlcohol,
        omegaFn: mw => parseFloat((mw / (mw - 2)).toFixed(3)),
        confidenceFn: () => 79,
        productSuffix: '(ketone)'
    },
    {
        id: 'sulfide_oxidation',
        label: 'Sulfide to sulfoxide',
        category: 'oxidation',
        conditions: ['oxidative'],
        test: f => f.hasSulfide,
        omegaFn: mw => parseFloat((mw / (mw + 16)).toFixed(3)),
        confidenceFn: () => 85,
        productSuffix: '(sulfoxide)'
    },
    {
        id: 'amine_oxidation',
        label: 'Primary amine N-oxidation',
        category: 'oxidation',
        conditions: ['oxidative'],
        test: f => f.hasAmine,
        omegaFn: mw => parseFloat((mw / (mw + 16)).toFixed(3)),
        confidenceFn: () => 71,
        productSuffix: '(N-oxide)'
    },
    {
        id: 'decarboxylation',
        label: 'Decarboxylation (loss of CO2)',
        category: 'thermal_photolytic',
        conditions: ['thermal', 'photolytic'],
        test: f => f.hasCarboxylicAcid,
        omegaFn: mw => parseFloat((mw / Math.max(1, mw - 44)).toFixed(3)),
        confidenceFn: () => 68,
        productSuffix: '(decarboxylated)'
    },
    {
        id: 'aromatic_hydroxylation',
        label: 'Aromatic ring hydroxylation',
        category: 'photolysis',
        conditions: ['photolytic', 'oxidative'],
        test: f => f.hasAromaticRing,
        omegaFn: mw => parseFloat((mw / (mw + 16)).toFixed(3)),
        confidenceFn: (mw, s) => s === 'photolytic' ? 72 : 65,
        productSuffix: '(hydroxylated ring)'
    },
    {
        id: 'ring_opening',
        label: 'Lactone ring opening',
        category: 'acid_hydrolysis',
        conditions: ['acid', 'base'],
        test: f => f.hasLactone,
        omegaFn: mw => parseFloat((mw / (mw + 18)).toFixed(3)),
        confidenceFn: () => 82,
        productSuffix: '(open chain)'
    }
];

function predictProducts(smiles, stressType, maxProducts = 5) {
    const features = parseSmilesFeatures(smiles);
    const parentMW = features.molecularWeight;
    const products = [];

    for (const rule of DEGRADATION_RULES) {
        if (!rule.conditions.includes(stressType)) continue;
        if (!rule.test(features)) continue;

        const productMW = Math.max(10, parentMW + (rule.id.includes('oxidation') ? 16 : -18));
        const omega = rule.omegaFn(parentMW);
        const confidence = rule.confidenceFn(parentMW, stressType);

        products.push({
            smiles: `${smiles.slice(0, 10)}...${rule.productSuffix}`,
            molecular_weight: productMW,
            omega,
            pathway: rule.label,
    ...
    // Sort by confidence desc, cap at maxProducts
    return products.sort((a, b) => b.confidence - a.confidence).slice(0, maxProducts);
}

function analyzeMolecule(smiles, stressType) {
    const features = parseSmilesFeatures(smiles);

    // Susceptibility scoring
    let score = 20; // base
    const reasons = [];
    if (features.hasEster && ['acid', 'base'].includes(stressType)) { score += 30; reasons.push('Ester group susceptible to hydrolysis'); }
    if (features.hasAmide && ['acid', 'base'].includes(stressType)) { score += 25; reasons.push('Amide bond hydrolysis-prone'); }
    if (features.hasSulfide && stressType === 'oxidative') { score += 35; reasons.push('Sulfide readily oxidized'); }
    if (features.hasAlcohol && stressType === 'oxidative') { score += 20; reasons.push('Alcohol susceptible to oxidation'); }
    if (features.hasAmine && stressType === 'oxidative') { score += 20; reasons.push('Amine N-oxidation possible'); }
    if (features.hasAromaticRing && ['photolytic', 'oxidative'].includes(stressType)) { score += 15; reasons.push('Aromatic ring prone to hydroxylation'); }
    if (features.hasCarboxylicAcid && ['thermal', 'photolytic'].includes(stressType)) { score += 25; reasons.push('Carboxylic acid may decarboxylate'); }
    if (stressType === 'photolytic') { score += 10; reasons.push('Photolytic conditions increase overall reactivity'); }
    if (reasons.length === 0) reasons.push('No highly reactive functional groups detected');

    score = Math.min(100, score);
    const level = score >= 70 ? 'HIGH' : score >= 40 ? 'MODERATE' : 'LOW';

    // Kinetics estimation
    const stressKConstants = { acid: 0.05, base: 0.04, oxidative: 0.08, thermal: 0.03, photolytic: 0.06 };
    const kBase = stressKConstants[stressType] || 0.05;
    const kAdjusted = kBase * (1 + score / 200);
    const halfLifeHours = Math.log(2) / kAdjusted;

    const mw = features.molecularWeight;
    const numAtoms = features.numAtoms;

    return {
        degradation_susceptibility: { susceptibility_score: score, level, reasons },
        kinetics: {
            rate_constant_k: parseFloat(kAdjusted.toFixed(5)),
            half_life_hours: parseFloat(halfLifeHours.toFixed(1)),
            half_life_days: parseFloat((halfLifeHours / 24).toFixed(1)),
            order: 'first'
        },
        molecular_descriptors: {
            molecular_weight: mw,
            logp: parseFloat(((numAtoms * 0.3) - 1.2).toFixed(2)),
            num_h_donors: [features.hasAlcohol, features.hasAmide, features.hasAmine, features.hasCarboxylicAcid].filter(Boolean).length,
            num_h_acceptors: [features.hasEster, features.hasAmide, features.hasAlcohol, features.hasCarboxylicAcid].filter(Boolean).length + 1,
            num_rotatable_bonds: Math.max(1, Math.floor(numAtoms / 4)),
            num_aromatic_rings: features.hasAromaticRing ? 1 : 0
        }
    };
}

function predictMassBalance(smiles, stressType, degradationPercent) {
    const products = predictProducts(smiles, stressType, 3);
    if (!products.length) {
        return {
            predicted_lk_imb: parseFloat((100 - degradationPercent).toFixed(2)),
            predicted_cimb: parseFloat((100 - degradationPercent).toFixed(2)),
            degradation_percent: degradationPercent,
            num_products_predicted: 0,
            note: 'No degradation products predicted for this stress type',
            major_products: []
        };
    }
    const perProduct = degradationPercent / products.length;
    const lkDegradantsContrib = products.reduce((sum, p) => sum + perProduct * p.omega, 0);
    const predictedLkImb = parseFloat(((100 - degradationPercent) + lkDegradantsContrib).toFixed(2));
    return {
        predicted_lk_imb: predictedLkImb,
        predicted_cimb: predictedLkImb,
        degradation_percent: degradationPercent,
        num_products_predicted: products.length,
        major_products: products.map(p => ({ pathway: p.pathway, omega: p.omega, confidence: p.confidence }))
    };
}

// --- Routes ------------------------------------------------------------

// GET /api/predict/example-molecules
app.get('/api/predict/example-molecules', (req, res) => {
    res.json({
        success: true,
        examples: [
            { name: 'Aspirin', smiles: 'CC(=O)Oc1ccccc1C(=O)O', category: 'NSAID', mw: 180.16 },
            { name: 'Acetaminophen', smiles: 'CC(=O)Nc1ccc(O)cc1', category: 'Analgesic', mw: 151.16 },
            { name: 'Ibuprofen', smiles: 'CC(C)Cc1ccc(cc1)C(C)C(=O)O', category: 'NSAID', mw: 206.28 },
            { name: 'Caffeine', smiles: 'Cn1c2c(c(=O)n(c1=O)C)n(cn2)C', category: 'Stimulant', mw: 194.19 },
            { name: 'Metformin', smiles: 'CN(C)C(=N)NC(=N)N', category: 'Antidiabetic', mw: 129.16 },
            { name: 'Atorvastatin', smiles: 'CC(C)c1nc2cc(F)ccc2c(CC(O)CC(O)CC(=O)O)c1-c1ccc(F)cc1', category: 'Statin', mw: 558.64 }
        ]
    });
});

// POST /api/predict/products
app.post('/api/predict/products', (req, res) => {
    try {
        const { smiles, stress_type } = req.body;
        if (!smiles) return res.status(400).json({ success: false, error: 'SMILES required' });

        const products = predictProducts(smiles, stress_type || 'oxidative');
        res.json({ success: true, result: { products, num_products: products.length, stress_type } });
    } catch (err) {
...
app.post('/api/predict/analyze', (req, res) => {
    try {
        ...
        const result = analyzeMolecule(smiles, stress_type || 'oxidative');
...
// POST /api/predict/mass-balance
app.post('/api/predict/mass-balance', (req, res) => {
    try {
        const { smiles, stress_type, degradation_percent } = req.body;
        if (!smiles) return res.status(400).json({ success: false, error: 'SMILES required' });

        const result = predictMassBalance(smiles, stress_type || 'oxidative', parseFloat(degradation_percent) || 10);
        res.json({ success: true, result });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
// POST /api/ml/gnn-predict  — tries Python gnnPredictor.py, falls back to JS simulation
app.post('/api/ml/gnn-predict', (req, res) => {
    const { smiles } = req.body;
    if (!smiles) return res.status(400).json({ success: false, error: 'SMILES required' });

    // Try Python GNN predictor first
    const pythonScript = path.join(__dirname, 'ml', 'gnnPredictor.py');
    const proc = spawn('python', [pythonScript, smiles]);
    let out = '';
    let err = '';
    let settled = false;
    ...
    const fallback = () => {
        if (settled) return;
        settled = true;
        // JS fallback: simulate atom lability from SMILES
        const features = parseSmilesFeatures(smiles);
        const atoms = (smiles.match(/[A-Z][a-z]*/g) || []).slice(0, 20);
        const labilityMap = { O: 0.65, N: 0.55, S: 0.80, C: 0.25, F: 0.15, Cl: 0.35, Br: 0.45, I: 0.50 };
        const atomLability = atoms.map((sym, i) => ({
            index: i,
            symbol: sym,
            lability: parseFloat(((labilityMap[sym] || 0.2) + Math.random() * 0.1).toFixed(3))
        }));
        const overallSusceptibility = atomLability.length
            ? parseFloat((atomLability.reduce((s, a) => s + a.lability, 0) / atomLability.length * 100).toFixed(2))
            : 30;
        res.json({
            success: true,
            source: 'js-fallback',
            overall_susceptibility: overallSusceptibility,
            atom_lability: atomLability,
            num_atoms: atomLability.length,
            model_type: 'Rule-based Lability Estimator'
        });
    };

    const timeout = setTimeout(fallback, 8000); // 8s timeout
    ...
    proc.on('close', code => {
        ...
        if (settled) return;
        // Only mark settled here if Python succeeded — otherwise let fallback() handle it
        if (code === 0 && out.trim()) {
            try {
                const result = JSON.parse(out);
                settled = true;
                return res.json({ ...result, source: 'python-gnn' });
            } catch (_) { /* fall through to JS fallback */ }
        }
        // Python failed/bad output — run JS fallback (it sets settled itself)
        fallback();
    });
    proc.on('error', () => { clearTimeout(timeout); fallback(); });
});
